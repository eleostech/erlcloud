...
select_single_response() ->
    expect_chain([{ok, "200", [], single_result_response()}]),

    Result = erlcloud_sdb:select("select"),
    ?assertEqual(undefined, proplists:get_value(next_token, Result)).

select_next_token() ->
    expect_chain([{ok, "200", [], only_token_response()},
                  {ok, "200", [], single_result_response()}]),
...
select_all_503() ->
    expect_chain([{ok, "503", [], ""}, {ok, "200", [], single_result_response()}]),
...
select_all_two_results() ->
    expect_chain([{ok, "200", [], single_result_and_token_response()},
                  {ok, "200", [], single_result_response("item1")}]),
